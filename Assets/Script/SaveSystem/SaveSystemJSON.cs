...
public static class SaveSystemJSON
{
    ...
    private static string achievementFilePath = Application.persistentDataPath + "/achievementData.json";
    private static string itemFilePath = Application.persistentDataPath + "/itemData.json";
    private static string encryptionKey = "MySecretKey12345";

    private static HashSet<string> excludedScenes = new HashSet<string>
    {
        "MainMenu","PlayerDeathLoading","StageLoading_1","GameStartLoding","StageSelect"
    };
    ...
    {
        if (excludedScenes.Contains(stageName)) return;
    ...
    public static void DataSaveClearedStage(string stageName)
    {
        ...
        else wrapper = new StageListWrapper();
    ...
    // 아이템 저장/로드
    public static void DataSaveItems(ItemDatabase itemDatabase)
    {
        if (itemDatabase == null) return;
        string json = JsonUtility.ToJson(new ItemSaveWrapper(itemDatabase), true);
        string encrypted = Encrypt(json, encryptionKey);
        File.WriteAllText(itemFilePath, encrypted);
    }

    public static void DataLoadItems(ItemDatabase itemDatabase)
    {
        if (!File.Exists(itemFilePath)) return;
        string encrypted = File.ReadAllText(itemFilePath);
        string decrypted = Decrypt(encrypted, encryptionKey);
        ItemSaveWrapper wrapper = JsonUtility.FromJson<ItemSaveWrapper>(decrypted);
        wrapper.ApplyToDatabase(itemDatabase);
    }

    // 데이터 초기화
    public static void ClearQuests() { if (File.Exists(questFilePath)) File.Delete(questFilePath); }
    public static void ClearStage() { if (File.Exists(stageFilePath)) File.Delete(stageFilePath); }
    public static void ClearAchievements() { if (File.Exists(achievementFilePath)) File.Delete(achievementFilePath); }
    public static void ClearItems() { if (File.Exists(itemFilePath)) File.Delete(itemFilePath); }
...
public class StageSaveWrapper
{
    ...
    public StageSaveWrapper(string stageName) { currentStageName = stageName; }
}
...
public class ItemSaveWrapper
{
    ...
    public ItemSaveWrapper(ItemDatabase database)
    {
    ...
    public void ApplyToDatabase(ItemDatabase database)
    {
        database.items.Clear();
        for (int i = 0; i < itemIDs.Count; i++)
        {
            database.items[itemIDs[i]] = itemCounts[i];
        }
    }
}